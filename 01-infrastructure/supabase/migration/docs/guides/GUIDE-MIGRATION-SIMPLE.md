...
**Migration simple (schéma par défaut `public`):**
```bash
./migrate.sh
```

**Migration avec schéma personnalisé (multi-projets):**
```bash
./migrate.sh --schema MonProjet
```
> 💡 **Note**: PostgreSQL convertit automatiquement le nom en minuscules (`monprojet`)

**Mode test (export uniquement, sans modifier le Pi):**
```bash
./migrate.sh --dry-run
```
...
### 📝 Si vous avez utilisé un schéma personnalisé

Dans Supabase Studio, **sélectionnez votre schéma** dans le dropdown en haut à gauche :
```
[Dropdown] : public → monprojet
```

**Important** : Mettez à jour votre code client :

```javascript
// ❌ AVANT (schéma public par défaut)
const supabase = createClient(url, anonKey)

// ✅ APRÈS (schéma personnalisé)
const supabase = createClient(url, anonKey, {
  db: {
    schema: 'monprojet'  // ⚠️ En minuscules !
  }
})
```

> 💡 **Pourquoi minuscules ?** PostgreSQL convertit automatiquement tous les noms de schémas non-quotés en minuscules. Même si vous avez tapé `--schema MonProjet`, le schéma créé sera `monprojet`.
