...
// =============================================================================
// HTTP Routes - Quick Launch Services Detection
// =============================================================================

// GET /api/quick-launch/services - Auto-detect available services from Docker
app.get('/api/quick-launch/services', ...authOnly, async (req, res) => {
  try {
    ...
    // Service definitions (icons, descriptions)
    const serviceConfig = {
    ...
    for (const line of lines) {
      ...
      // Parse ports (format: 0.0.0.0:8000->8000/tcp, 127.0.0.1:3000->3000/tcp)
      ...
      for (const match of portMatches) {
        ...
        services.push({
          ...
          needsTunnel: isLocalhost, // True if 127.0.0.1 only
    ...
    res.json({ services: [] }); // Return empty array on error (fallback to client defaults)
  }
});
