...
# Dépendances : Docker/Compose de Week1. Ports : 3000 (Studio), 8001 (API/Kong).
# Usage : sudo SUPABASE_PORT=8001 ./setup-week2-supabase-final.sh
# Actions Manuelles Post-Script : Accéder Studio http://IP:3000, créer projet, noter API keys.
# =============================================================================
...
SCRIPT_VERSION="2.5.2-post-reboot-fix"
...
check_prereqs() {
  ...
  local entropy=$(cat /proc/sys/kernel/random/entropy_avail); [[ $entropy -ge 256 ]] || warn "Entropie faible ($entropy) - RNG OK mais monitor"
...
setup_project_dir() {
  ...
  rm -rf "$PROJECT_DIR"  # Cleanup ancien (attention: perte volumes si FORCE=0)
...
generate_secrets() {
  ...
  local site_url="http://${HOSTNAME:-localhost}:$SUPABASE_PORT"  # Auto-detect IP fallback
...
deploy_supabase() {
  ...
  for i in {1..6}; do sleep 5; docker compose ps | grep -q "healthy\|Up" && break; done
...
validate_deployment() {
  ...
  # Test Studio
  curl -s http://localhost:3000 >/dev/null && ok "Studio OK (3000)" || warn "Studio en bootstrap"
  # Test API Kong
  curl -s http://localhost:$SUPABASE_PORT >/dev/null && ok "API OK ($SUPABASE_PORT)" || warn "API en bootstrap"
  # Test PG
  docker compose exec -T postgresql pg_isready >/dev/null && ok "PG OK" || error "PG KO"
  # Affichage keys
...
echo "Prochaine Étape: Script 4 - Nginx Reverse Proxy + HTTPS (Caddy ou Certbot)."