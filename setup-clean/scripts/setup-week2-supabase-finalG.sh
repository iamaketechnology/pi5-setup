...
# Version : 2.6.14-schema-owner-fix (Corrections: ALTER OWNER pour _realtime; verbose psql; pré-compile Realtime code)  
...
# Corrections v2.6.14 basées sur logs (20/09/2025):  
# - Fix "Échec schema _realtime": Ajout ALTER OWNER TO postgres; verbose output psql (logs erreurs); retry 3x.  
# - Seeding: Pré-compile (deps.get + compile) code Realtime; mix run -e pour seeds (réseau Docker).  
# - ARM64: Buffers PG 128MB; ulimits 262144; relance +5 unhealthy.  
# - Recherche: GitHub #372/#748 (owner _realtime); StackOverflow (migrations via ALTER).  
# =============================================================================  
...
SCRIPT_VERSION="2.6.14-schema-owner-fix"  
...
# Création du dossier projet et volumes + clone/Pré-compile Realtime pour seeding  
setup_project_dir() {  
  ...
  # Clone Realtime repo pour seeding dev (v2.6.14)  
  ...
  if [[ ! -d "$realtime_code_dir" || "$FORCE_RECREATE" == "1" ]]; then  
    ...
    # Pré-compile pour speed seeding (deps.get + compile, v2.6.14)  
...
POSTGRES_SHARED_BUFFERS=128MB  # Ultra-conservateur anti-OOM Pi5 (v2.6.14)  
...
ENV  
  ...
  ok ".env créé - JWT base64 + APP_NAME (buffers 128MB, ulimits 262144)."  
}  
...
# Création robuste de schémas (verbose + ALTER OWNER + retry, v2.6.14)  
...
# Initialisation migrations (auth + realtime schema/WAL ; entrypoint gère ecto)  
init_auth_migrations() {  
  ...
  # Schémas robustes (auth + extensions + realtime avec owner, v2.6.14)  
...
# Seeding Realtime via conteneur Elixir éphémère (pré-compilé, mix run -e, v2.6.14)  
run_realtime_seeding() {  
  ...
  # Run seeding dans réseau Docker (mix run -e, v2.6.14)  
...
deploy_supabase() {  
  ...
  for i in $(seq 1 "$max_wait"); do  
    ...
    if [[ $relance_count -ge 5 ]]; then  # +1 vs v2.6.13  
